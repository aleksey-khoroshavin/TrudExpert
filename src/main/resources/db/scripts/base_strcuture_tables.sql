...
alter table organizations
    drop constraint if exists UK_p9pbw3flq9hkay8hdx3ypsldy;

alter table organizations
    add constraint UK_p9pbw3flq9hkay8hdx3ypsldy unique (name);

alter table organization_agent
    add constraint FK8on0571i3qc52aotxtsml0uxt foreign key (organization_id) references organizations;

alter table organization_listener
    add constraint FKbue895lh22b581ptrmkh3vext foreign key (listener_id) references listeners;

alter table organization_listener
    add constraint FK5pbbkj8kxx3gh170tcm58i2vc foreign key (organization_id) references organizations;